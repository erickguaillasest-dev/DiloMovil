...
class ZoeSpeechHelper(private val context: Context) {
    ...
    companion object {
        ...
        private val ACENTOS_DISPONIBLES = listOf(
            AcentoVoz("es", "AR", "acento argentino", listOf(
                "acento argentino", "habla como argentino", "habla como argentina",
                "hablame como argentino", "hablame como argentina", "con acento argentino",
                "voz argentina", "como che", "habla como che", "ponte argentino", "ponte argentina",
                "suena argentino", "suena argentina", "vuelve a tu acento", "acento normal", "acento original"
            )),
            AcentoVoz("es", "MX", "acento mexicano", listOf(
                "acento mexicano", "habla como mexicano", "habla como mexicana",
                "hablame como mexicano", "con acento mexicano", "voz mexicana", "suena mexicano"
            )),
            AcentoVoz("es", "ES", "acento español", listOf(
                "acento español", "habla como español", "habla como española",
                "con acento español", "voz española", "suena español", "acento de españa"
            )),
            AcentoVoz("es", "US", "acento neutro", listOf(
                "acento neutro", "voz neutra", "habla neutro"
            )),
            AcentoVoz("es", "EC", "acento ecuatoriano", listOf(
                "acento ecuatoriano", "habla como ecuatoriano"
            ))
        )
    ...
    fun activarAcento(acento: AcentoVoz): String {
        val tts = textToSpeech ?: return "Todavía no tengo la voz lista, dame un segundo e inténtalo de nuevo."
        val locale = Locale(acento.idioma, acento.pais)
        val disponibilidad = tts.isLanguageAvailable(locale)
        return if (disponibilidad == TextToSpeech.LANG_MISSING_DATA || disponibilidad == TextToSpeech.LANG_NOT_SUPPORTED) {
            "Este dispositivo no tiene instalada la voz con ${acento.nombre}. Ve a Ajustes > Accesibilidad > " +
                    "Salida de texto a voz para descargar más idiomas, o dime otro acento."
        } else {
            tts.setLanguage(locale)
            acentoActual = acento
            "Listo, che. Ahora te hablo con ${acento.nombre}. ¿Cómo se escucha?"
        }
    }
    ...
    fun inicializar(onListo: (() -> Unit)? = null, onFallo: ((String) -> Unit)? = null) {
        textToSpeech = TextToSpeech(context) { status ->
            ...
            var resultado = tts?.setLanguage(Locale(acentoActual.idioma, acentoActual.pais))
            if (resultado == TextToSpeech.LANG_MISSING_DATA || resultado == TextToSpeech.LANG_NOT_SUPPORTED) {
                resultado = tts?.setLanguage(Locale("es", "AR"))
            }
            if (resultado == TextToSpeech.LANG_MISSING_DATA || resultado == TextToSpeech.LANG_NOT_SUPPORTED) {
    ...
    fun escuchar(onResultado: (String) -> Unit, onError: (String) -> Unit, onEmpezoAEscuchar: (() -> Unit)? = null) {
        ...
        val intent = Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).apply {
            ...
            putExtra(RecognizerIntent.EXTRA_LANGUAGE, "${acentoActual.idioma}-${acentoActual.pais}")
